...
enum { MAX_ASSET_HANDLERS = 512, MAX_PATH_VARIABLES = 512 };
...
void asset_add_path_variable(fpath variable, fpath mapping) {
  ...
  if (num_path_variables == MAX_PATH_VARIABLES) {
    error("Already reached maximum num of path variables (%i)",
          MAX_PATH_VARIABLES);
  }
...
fpath asset_hndl_path(asset_hndl *ah) { return ah->path; }
...
void asset_cache_flush(void) { asset_timestamp = SDL_GetTicks(); }
...
void asset_handler_cast(type_id type, const char *extension,
                        void *asset_loader(const char *filename),
                        void asset_deleter(void *asset)) {

  if (num_asset_handlers == MAX_ASSET_HANDLERS) {
    warning("Max number of asset handlers reached. Handler for extension '%s' "
            "not added.",
            extension);
    return;
  }
